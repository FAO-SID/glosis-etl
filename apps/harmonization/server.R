# =============================================================================
# SERVER.R - Server Logic for GLOSIS-ETL Harmonization Shiny App hoy
# =============================================================================
# This file contains all the reactive logic, data processing, and output
# rendering for the application. It handles file uploads, data validation,
# quality checks, and Excel workbook generation.
# =============================================================================



# ---------- Runtime / Debug (Docker-safe) ----------
# Ensure temp dir exists and is writable
Sys.setenv(TMPDIR = "/tmp", TEMP = "/tmp", TMP = "/tmp")
dir.create("/tmp", showWarnings = FALSE, recursive = TRUE)

# Pick a writable directory for logs/checkpoints (prefer Shiny Server data dir)
.pick_writable_dir <- function(candidates) {
  for (d in candidates) {
    ok <- tryCatch({
...
.LOG_DIR <- .pick_writable_dir(c("/var/lib/shiny-server", "/srv/shiny-server", "/tmp"))
.LOG_FILE <- file.path(.LOG_DIR, "download_debug.log")
.CHK_DIR  <- file.path(.LOG_DIR, "wb_checkpoints")
dir.create(.CHK_DIR, showWarnings = FALSE, recursive = TRUE)

dbg <- function(...) {
  msg <- paste0(format(Sys.time(), "%F %T"), " | ", paste(..., collapse = " "), "\n")
  tryCatch(cat(msg, file = .LOG_FILE, append = TRUE), error = function(e) {
    cat(msg) # fallback to stdout
  })
}
# Log where debug artifacts are written
try(dbg('INIT', 'LOG_DIR:', .LOG_DIR, '| CHK_DIR:', .CHK_DIR), silent = TRUE)


# Save a checkpoint workbook to help pinpoint where corruption happens
# Writes to: <LOG_DIR>/wb_checkpoints/chk_<tag>.xlsx
safe_save_check <- function(wb, tag) {
  out <- file.path(.CHK_DIR, paste0("chk_", tag, ".xlsx"))
  dbg("SAFE_SAVE_START", tag, "->", out)
  size <- tryCatch({
    callr::r(
      function(wb, out) {
        Sys.setenv(TMPDIR = "/tmp", TEMP = "/tmp", TMP = "/tmp")
        dir.create("/tmp", showWarnings = FALSE, recursive = TRUE)
        openxlsx::saveWorkbook(wb, file = out, overwrite = TRUE)
        as.numeric(file.info(out)$size)
      },
      args = list(wb = wb, out = out),
      show = FALSE
    )
  }, error = function(e) {
    dbg("SAFE_SAVE_FAIL", tag, "|", conditionMessage(e))
    stop(sprintf("safe_save_check failed at '%s': %s", tag, conditionMessage(e)))
  })
  dbg("SAFE_SAVE_OK", tag, "| size:", size)
  invisible(out)
}

# ---------- Helpers ----------
# Define your keyword list
keywords <- c("bd", "bulk_density", "soc", "som", "c_tot", "ph", "ph_h2o", "ph_water", 
...
clear_region <- function(wb, sheet, rows, cols) {
  # NOTE: On XLSX templates, openxlsx::deleteData() can corrupt sheet structures
  # (tables, merges, styles) and later crash saveWorkbook(). We "wipe" cells by
  # writing NAs instead, which is safer.
  if (!(sheet %in% names(wb))) return(invisible(NULL))
  if (length(rows) == 0 || length(cols) == 0) return(invisible(NULL))
  r0 <- min(rows); c0 <- min(cols)
  nr <- length(rows); nc <- length(cols)
  if (nr <= 0 || nc <= 0) return(invisible(NULL))
  
  blank <- matrix(NA, nrow = nr, ncol = nc)
  openxlsx::writeData(
    wb, sheet = sheet, x = blank,
...
server <- function(input, output, session) {
  ...
  procedures_lookup_raw <- reactive({
    read_procedures_csv("glosis_procedures_v2.csv")
  })
  ...
  template_path <- reactive({
    default <- "glosis_template_v6.xlsx"
  ...
  output$status <- renderPrint({
    if (is.null(input$data_file)) {
      ...
      cat("Default template exists:", file.exists("glosis_template_v6.xlsx"), "\n")
      cat("Procedures lookup exists:", file.exists("glosis_procedures_v2.csv"), "\n")
      return()
    }
  ...
  output$validation_panel <- renderUI({
    ...
    if (!file.exists("glosis_procedures_v2.csv")) msgs <- c(msgs, "glosis_procedures_v2.csv not found in app folder.")
  ...
  build_workbook <- reactive({
    tryCatch({
      ...
      # Construct rename list dynamically based on inputs
      cols_to_rename <- c(
        X      = input$col_lon,
        Y      = input$col_lat,
        texid  = input$col_texid,
        top    = input$col_top,
        bottom = input$col_bottom
      )
      
      # Optional horizon column
      if (!is.null(input$col_hor) && nzchar(input$col_hor)) {
        cols_to_rename["hor"] <- input$col_hor
      }
      
      # Apply renaming using any_of() to be safe
      df2 <- df %>% rename(any_of(cols_to_rename))
      
      # Ensure 'hor' column exists (fill with NA if missing) for downstream logic
      if (!("hor" %in% names(df2))) {
        df2$hor <- NA_character_
      }
      ...
      # ---- Coerce to base data.frame (Docker-safe for openxlsx) ----
      to_df <- function(x) as.data.frame(x, stringsAsFactors = FALSE)
      plot_info          <- to_df(plot_info)
      profile_info       <- to_df(profile_info)
      element_info       <- to_df(element_info)
      specimen_info      <- to_df(specimen_info)
      original_data_info <- to_df(original_data_info)
      procedures_info    <- to_df(procedures_info)
      metadata_info      <- to_df(metadata_info)
      
      if ("date" %in% names(plot_info)) {
        plot_info$date <- as.Date(plot_info$date)
      }
      
      dbg("BUILD_WORKBOOK: prepared dfs | plots:", nrow(plot_info),
          "| specimens:", nrow(specimen_info),
          "| props:", length(soil.properties))
      
      # Template
      tp <- template_path()
      validate(need(file.exists(tp), paste("Template not found:", tp)))
      wb <- openxlsx::loadWorkbook(tp)
      
      .do_chk   <- identical(Sys.getenv("GLOSIS_WB_CHECKPOINTS", "0"), "1")
      .do_clear <- identical(Sys.getenv("GLOSIS_WB_CLEAR", "0"), "1")
      if (.do_chk) safe_save_check(wb, "00_loaded")
      
      .clr <- function(sheet, startRow, startCol, nr, nc, pad_rows = 50L, pad_cols = 10L) {
        if (!.do_clear) return(invisible(NULL))
        nr <- as.integer(nr %||% 0); nc <- as.integer(nc %||% 0)
        r_end <- startRow + max(nr, 0L) + pad_rows
        c_end <- startCol + max(nc, 0L) + pad_cols
        clear_region(wb, sheet, rows = startRow:r_end, cols = startCol:c_end)
        invisible(NULL)
      }
      
      required_sheets <- c("Plot Data","Profile Data","Element Data","Specimen Data",
                           "Original Data","Procedures","Metadata")
      missing <- setdiff(required_sheets, names(wb))
      validate(need(length(missing) == 0,
                    paste("Template missing sheets:", paste(missing, collapse = ", "))))
      
      # IMPORTANT: avoid deleteData() on templates; clear_region() now "wipes" cells by writing NAs.
      .clr("Plot Data", 3, 1, nrow(plot_info), ncol(plot_info))
      openxlsx::writeData(wb, "Plot Data", x = plot_info, startRow = 3, startCol = 1,
                          colNames = FALSE, rowNames = FALSE)
      if (.do_chk) safe_save_check(wb, "01_plot_written")
      
      # Date style only if we actually wrote rows
      n <- nrow(plot_info)
      if (n > 0) {
      ...
      if (.do_chk) safe_save_check(wb, "02_plot_styled")
      
      .clr("Profile Data", 3, 1, nrow(profile_info), ncol(profile_info))
      openxlsx::writeData(wb, "Profile Data", x = profile_info, startRow = 3, startCol = 1,
                          colNames = FALSE, rowNames = FALSE)
      if (.do_chk) safe_save_check(wb, "03_profile")
      
      .clr("Element Data", 3, 1, nrow(element_info), ncol(element_info))
      openxlsx::writeData(wb, "Element Data", x = element_info, startRow = 3, startCol = 1,
                          colNames = FALSE, rowNames = FALSE)
      if (.do_chk) safe_save_check(wb, "04_element")
      
      .clr("Specimen Data", 1, 1, nrow(specimen_info) + 1L, ncol(specimen_info))
      openxlsx::writeData(wb, "Specimen Data", x = specimen_info, startRow = 1, startCol = 1,
                          colNames = TRUE, rowNames = FALSE)
      if (.do_chk) safe_save_check(wb, "05_specimen")
      
      .clr("Original Data", 1, 1, nrow(original_data_info) + 1L, ncol(original_data_info))
      openxlsx::writeData(wb, "Original Data", x = original_data_info, startRow = 1, startCol = 1,
                          colNames = TRUE, rowNames = FALSE)
      if (.do_chk) safe_save_check(wb, "06_original")
      
      .clr("Procedures", 2, 1, nrow(procedures_info), ncol(procedures_info))
      openxlsx::writeData(wb, "Procedures", x = procedures_info, startRow = 2, startCol = 1,
                          colNames = FALSE, rowNames = FALSE)
      if (.do_chk) safe_save_check(wb, "07_procedures")
      
      .clr("Metadata", 3, 1, nrow(metadata_info), ncol(metadata_info))
      openxlsx::writeData(wb, "Metadata", x = metadata_info, startRow = 3, startCol = 1,
                          colNames = FALSE, rowNames = FALSE)
      if (.do_chk) safe_save_check(wb, "08_metadata")
      
      dbg("BUILD_WORKBOOK: complete")
  ...
  # ---- XLSX: generate first, then download ----
  # Why: downloadHandler only starts sending bytes AFTER content() finishes.
  # If workbook generation takes a long time, browsers often cancel/retry and you see
  # "Failed - Network error" even though the server eventually finished.
  xlsx_cache_path <- reactiveVal(NULL)
  
  invalidate_xlsx_cache <- function() {
    old <- xlsx_cache_path()
    if (!is.null(old) && file.exists(old)) unlink(old)
    xlsx_cache_path(NULL)
    invisible(NULL)
  }
  
  # Invalidate cache when key inputs change
  observeEvent(input$data_file, invalidate_xlsx_cache(), ignoreInit = TRUE)
  observeEvent(input$soil_props, invalidate_xlsx_cache(), ignoreInit = TRUE)
  observeEvent(input$metadata_edit_dt_cell_edit, invalidate_xlsx_cache(), ignoreInit = TRUE)
  
  # Build + save the XLSX on demand (NOT inside the download request)
  observeEvent(input$prepare_xlsx, {
    req(input$data_file)
    
    invalidate_xlsx_cache()
    
      # Show blocking modal with a "running wheel" spinner for better visibility
      showModal(modalDialog(
        title = NULL, # No title bar for cleaner look, or "Processing..."
        div(
          style = "text-align: center; padding: 20px;",
          icon("spinner", class = "fa-spin fa-3x", style = "color: #3c8dbc;"),
          tags$h4("Generating XLSX...", style = "margin-top: 15px; font-weight: bold;"),
      ...
      tryCatch({
        dbg("PREPARE_XLSX_START")
        
        # Build logic (removed incremental progress calls as we use modal now)
        wb <- build_workbook()
        
        tmp <- tempfile(fileext = ".xlsx")
        
        use_callr <- identical(Sys.getenv("GLOSIS_USE_CALLR_SAVE", "1"), "1")
        if (use_callr) {
          size <- callr::r(
            function(wb, tmp) {
              Sys.setenv(TMPDIR = "/tmp", TEMP = "/tmp", TMP = "/tmp")
              ...
              as.numeric(file.info(tmp)$size)
            },
            args = list(wb = wb, tmp = tmp),
            show = FALSE
          )
        } else {
          openxlsx::saveWorkbook(wb, file = tmp, overwrite = TRUE)
        ...
        if (!file.exists(tmp) || is.na(size) || size < 2000) {
          stop("XLSX not created correctly (tmp missing or too small).")
        }
        
        xlsx_cache_path(tmp)
        dbg("PREPARE_XLSX_DONE", "| tmp:", tmp, "| size:", size)
        
        removeModal() # Close loading modal
        ...
        showModal(modalDialog(
          ...
          div(
            ...
            tags$h4("XLSX Generated Successfully", style = "font-weight: bold; margin-bottom: 10px;"),
            tags$p("The file is ready. Please click the 'Download XLSX' button below.")
          ),
        ...
        removeModal() # Close modal on error
        dbg("PREPARE_XLSX_ERROR", "|", conditionMessage(e))
        showNotification(paste("XLSX generation failed:", conditionMessage(e)),
                         type = "error", duration = 15)
      })
  }, ignoreInit = TRUE)
  
  # UI: show download button only when ready (see ui.R)
  output$download_xlsx_ui <- renderUI({
    if (is.null(xlsx_cache_path())) {
      # Return nothing (button hidden) - instructions are now static in UI
      return(NULL)
    } else {
      downloadButton(
        "download_xlsx",
  ...
  # Fast download: just copy the already-prepared file
  output$download_xlsx <- downloadHandler(
    ...
    content = function(file) {
      ...
      dbg("DOWNLOAD_START", "| cache:", xlsx_cache_path(), "| target:", file)
      ok <- file.copy(xlsx_cache_path(), file, overwrite = TRUE)
      final_size <- suppressWarnings(as.numeric(file.info(file)$size))
      
      dbg("DOWNLOAD_COPY", "| ok:", ok, "| final_size:", final_size)
      
      if (!isTRUE(ok) || is.na(final_size) || final_size < 2000) {
        stop("Failed to stage cached XLSX for download.")
      }
